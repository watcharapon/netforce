...
class Test(TestCase):
    ...
    _description="Product purchase invoice and payment"
    ...
    def test_run(self):
        ...
        vals={
            ...
            "lines": [("create",{
                ...
                "qty": 1,
                "unit_price": 1000,
                "account_id": get_model("account.account").search([["name","=","Purchase"]])[0],
                "tax_id": get_model("account.tax.rate").search([["name","=","Purchase Goods"]])[0],
                "amount": 1000,
            })],
        }
        ...
        self.assertEqual(inv.tax_no,"1234")
        move=inv.move_id
        ...
        self.assertEqual(move.lines[1].account_id.name,"Input VAT")
        self.assertEqual(move.lines[1].debit,70)
        self.assertEqual(move.lines[1].partner_id.id,vals["partner_id"])
        self.assertEqual(move.lines[1].tax_comp_id.name,"Input VAT")
        self.assertEqual(move.lines[1].tax_base,1000)
        self.assertEqual(move.lines[1].tax_no,"1234")
        self.assertEqual(move.lines[2].account_id.name,"Purchase")
